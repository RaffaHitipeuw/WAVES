...
def test_water_level_reading():
    ...
    return (
        reading.node_id == "NODE-001" and
        reading.water_level == 25.5 and
        reading.source == DataSource.SIMULATOR
    )
...
def test_reading_to_dict():
    reading = WaterLevelReading(node_id="TEST", water_level=30.0)
    d = reading.to_dict()
    return (
        "id" in d and
        "nodeId" in d and
...
def test_monitoring_node():
    ...
    return (
        node.id == "NODE-001" and
        node.thresholds["watch"] == 30
    )
...
def test_simulator_creation():
    sim = WaterLevelSimulator(SimulatorConfig(
        node_id="TEST",
        start_level=20,
        mode="rising"
    ))
    return (
        sim.node_id == "TEST" and
        sim.current_level == 20 and
        sim.mode == "rising"
    )
...
def test_simulator_generate_reading():
    sim = WaterLevelSimulator(SimulatorConfig(start_level=20))
    sim.start()
    reading = sim._generate_reading()
    sim.stop()
    return reading is not None and reading.water_level >= 0
...
def test_simulator_modes():
    modes = ["rising", "falling", "stable", "fluctuating", "rapid"]
    for mode in modes:
        sim = WaterLevelSimulator(SimulatorConfig(mode=mode, start_level=25))
        sim.start()
        reading = sim._generate_reading()
        sim.stop()
        if reading is None:
...
def test_simulator_presets():
    ...
    for preset in ["demo", "rapid", "slow", "stable"]:
...
def test_engine_creation():
    node = MonitoringNode(id="NODE-001", name="Test Node")
...
def test_engine_process():
    engine = CoreEngine()
    reading = WaterLevelReading(node_id="NODE-001", water_level=25.0)
    result = engine.process(reading)
    return (
        result is not None and
        "reading" in result and
        result["reading"]["waterLevel"] == 25.0
    )
...
def test_engine_buffer():
    ...
    for level in [20, 21, 22, 23, 24]:
        reading = WaterLevelReading(node_id="NODE-001", water_level=float(level))
...
def test_end_to_end():
    ...
    return (
        len(readings_received) >= 5 and
        engine.state.readings_processed >= 5
    )
