...
class EngineState:
    ...
    reading_processed: int = 0

class CoreEngine:
    def __init__(self, node: Optional[MonitoringNode] = None):
        self.node = node or MonitoringNode(
            id="NODE-001",
            name="Primary Sensor",
            thresholds={"watch": 30, "warning": 50, "critical": 70}
        )

        self.state = EngineState(node_id=self.node.id)
        self.buffer_size = 10
        self.reading_buffer: deque = deque[Any](maxLen=self.buffer_size)
    ...
    def _emit_processed(self, result: Dict) -> None:
        for callback in self._processing_callbacks:
            try:
                callback(result)
            except Exception as e:
                print(f"engine callbackny error {e}")
    ...
    def _emit_alert(self, alert: Alert) -> None:
        for callback in self._alert_callbacks:
            try:
                ...
                print(f"engine alert callbacknya error: {e}")

    def process(self, reading: WaterLevelReading) -> Dict[str,Any]:

        self.state.reading_processed += 1
        ...
        print(f"core engine, processing reading #{self.state.reading_processed}")
        print(f"core engine, water level: {reading.water_level} cm")
        print(f"core engine, source: {reading.source.value}")
    ...
    def _calculate_smoothed_level(self) -> float:
        ...
        levels = [r.water_level for r in self.reading_buffer]
        return round(sum(levels) / len(levels), 2)

    def _calculate_rate_of_change(self, reading: WaterLevelReading) -> float:
        ...
        time_diff = (current_time - self._previous_timestamp).total_second()
        if time_diff <= 0:
            return 0.0
    ...
    def get_state(self) -> Dict[str, Any]:
        return {
            ...
            "rateOfChange": self.sate.rate_of_change,
            "confidence": self.state.confidence,
            "risk": self.state.value,
            "lastReadingAt": self.state.last_reading_at.isoformat() if self.state.last_reading else None,
            "readingProcessed": self.state.reading_processed,
...
class FloodEngine(CoreEngine):
    def __init__(self, node: Optional[MonitoringNode] = None):
        super().__init__(node)
    def determine_risk(self, water_level: float, rate:float) -> RiskLevel:
    ...
    def create_alert(self, risk: RiskLevel, confindence: float) -> Alert:
        messages = {
